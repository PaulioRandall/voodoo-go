	...
	return []scanTest{
		...
		scanTest{
			...
			Expect: []token.Token{
				...
				token.Token{`do`, 11, 13, 0, token.KEYWORD_FUNC},
				token.Token{`(`, 13, 14, 0, token.PAREN_CURVY_OPEN},
				token.Token{`a`, 14, 15, 0, token.IDENTIFIER_EXPLICIT},
				token.Token{`,`, 15, 16, 0, token.SEPARATOR_VALUE},
				token.Token{` `, 16, 17, 0, token.WHITESPACE},
				token.Token{`b`, 17, 18, 0, token.IDENTIFIER_EXPLICIT},
				token.Token{`)`, 18, 19, 0, token.PAREN_CURVY_CLOSE},
				token.Token{` `, 19, 20, 0, token.WHITESPACE},
				token.Token{`r`, 20, 21, 0, token.IDENTIFIER_EXPLICIT},
				token.Token{`,`, 21, 22, 0, token.SEPARATOR_VALUE},
				token.Token{` `, 22, 23, 0, token.WHITESPACE},
				token.Token{`err`, 23, 26, 0, token.IDENTIFIER_EXPLICIT},
				token.Token{`     `, 26, 31, 0, token.WHITESPACE},
			},
		},
		scanTest{
			TestLine: fault.CurrLine(),

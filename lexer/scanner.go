...
func ScanLine(line string, lineNum int) (r []lexeme.Symbol, lxErr LexError) {
...
func wordSym(itr *runer.RuneItr) *lexeme.Symbol {
	...
	case `scroll`:
		symType = lexeme.KEYWORD_SCROLL
	case `spell`:
		symType = lexeme.KEYWORD_SPELL
	case `loop`:
		symType = lexeme.KEYWORD_LOOP
	case `when`:
		symType = lexeme.KEYWORD_WHEN
	case `end`:
		symType = lexeme.KEYWORD_END
	case `key`:
		symType = lexeme.KEYWORD_KEY
	case `val`:
		symType = lexeme.KEYWORD_VAL
	case `true`:
		symType = lexeme.BOOLEAN
	case `false`:
		symType = lexeme.BOOLEAN
	default:
		symType = lexeme.VARIABLE
	}
...
// - literal number
func numSym(itr *runer.RuneItr) (s *lexeme.Symbol, err LexError) {
...
func spaceSym(itr *runer.RuneItr) *lexeme.Symbol {
...
// - go function call
func sourcerySym(itr *runer.RuneItr) (s *lexeme.Symbol, err LexError) {
...
// - string literal
func strSym(itr *runer.RuneItr) (s *lexeme.Symbol, err LexError) {
...
// - comment
func commentSym(itr *runer.RuneItr) *lexeme.Symbol {
...
func otherSym(itr *runer.RuneItr) (s *lexeme.Symbol, err LexError) {

	start := itr.Index()
	symType := lexeme.UNDEFINED
	runeCount := 0

	set := func(t lexeme.SymbolType, totalRunes int) {
		symType = t
